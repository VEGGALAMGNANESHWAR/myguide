...
def index(request):
    ...
    if not(products):
        messages.info(request, 'No Rooms Booked  Yet!')
...
def search(request):
    ...
    if not(products):
        messages.info(request, 'No Rooms Found!')
...
def add_to_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(product__pk=pk).exists():
            messages.info(request, "this is already in the Check-in List")
            return redirect("item_description", pk=pk)

        else:
            order.items.add(order_item)
            messages.info(request, "Successfully added in the in Checked-in List ,YOU CAN GOTO CHECK-IN page and go for payment !")
            return redirect("item_description", pk=pk)
        ...
        try:
            ...
            if result is not None:
                ...
                if payment_status is not None:
                    ...
                    request.session[
                        "order_complete"
                    ] = "You have booked your Slot Successfully! you can go to the Hostel on day!"
                    ...
                    try:
                        ...
                        sms = f"Congratulations {poster_name}! Your Room '{item_name}' has been booked by {buyer_name}. IO Team  pay you ₹{item_price} when Coustmer checked-in. Regards from IO"
